...
A blocked or deleted chat is the only way the bot learns someone left, because
Telegram sends no event for being blocked, so a terminal send error unsubscribes them, and
...
Telegram immediately, because an update left unacknowledged is one Telegram sends again,
